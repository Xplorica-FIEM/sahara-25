...
let recentPaymentsCache: { data: ApiDonation[]; expiresAt: number } | null =
    null;
...
async function fetchRecentPayments(): Promise<ApiDonation[]> {
    ...
    recentPaymentsInflight = fetch(RECENT_API_ENDPOINT, {
        ...
        .then(async (res) => {
            if (!res.ok)
                throw new Error(
                    `Failed to load recent payments: ${res.status}`
                );
            const json = (await res.json()) as TopPaymentsResponse;
            const data = Array.isArray(json?.data) ? json.data : [];
...
const InfiniteScrollDonors: React.FC<InfiniteScrollDonorsProps> = ({
    ...
    const [recentRows, setRecentRows] = React.useState<ApiDonation[] | null>(
        null
    );
        ...
        fetchTopPayments()
            ...
            .catch((e: any) => {
                if (mounted)
                    setError(
                        (prev) =>
                            prev ?? e?.message ?? "Failed to load top payments"
                    );
            });
