...
async function fetchTopPayments(): Promise<ApiDonation[]> {
    ...
    topPaymentsInflight = fetch(API_ENDPOINT, {
        ...
        .catch((error) => {
            ...
            console.info("ℹ️ To fix this error, ensure your backend server is running on localhost:5000 or set VITE_API_BASE_URL environment variable");
...
async function fetchRecentPayments(): Promise<ApiDonation[]> {
  ...
  recentPaymentsInflight = fetch(RECENT_API_ENDPOINT, {
    ...
    .catch((error) => {
      console.warn("🔌 Backend API not available for recent payments:", error.message);
      // Return empty array when backend is not available
      recentPaymentsCache = { data: [], expiresAt: now + CACHE_TTL_MS };
      return [];
    })
